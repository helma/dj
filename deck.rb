...
osc = Thread.fork do
  OSC.run do
    ...
    oscserver.add_pattern "/sixteenth" do |*args| 
      ...
      if (sixteenth % (8*16)) == 0
        eightbars = sixteenth/(8*16)
        col = eightbars % 8
        row = eightbars / 8
        led_8bars = col+16*row
        if eightbars < nr_8bars
          midiout.puts(144,last8,AMBER_LOW) if last8 and last8 != led_8bars 
          midiout.puts(144,led_8bars,GREEN_FULL)
          last8 = led_8bars
        else
          midiout.puts(144,last8,AMBER_LOW) if last8
  ...
  midiin.gets.each do |m|
    ...
    if d[0] == 144
      ...
      if file and File.exists?(file)
        ...
        nr_8bars = stat["Length (seconds)"].to_f*bpm/(8*240)
        n = 0
        (0..3).each do |r|
          (0..7).each do |c| 
